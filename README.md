...
```txt
django-cid==1.3.1          # Correlation ID middleware
```
...
class LogRequestResponseMiddleware(MiddlewareMixin):
    """
    Logs incoming requests (➡️) and outgoing responses (⬅️) with correlation_id.
    """
    ...
    def process_response(self, request, response):
        ...
        logger.info(
            f"⬅️ {response.status_code} {request.method} {request.path}",
            extra=log_entry
        )
...
services:
  app:
    ...
    environment:
      ...
      - USE_ELK_LOGGING=${USE_ELK_LOGGING}
...
**Purpose:**
- `uwsgi_param HTTP_X_CORRELATION_ID $http_x_correlation_id` passes the header to Django
- If the client doesn't send X-Correlation-Id, nginx passes an empty header
- Django's `CidMiddleware` will generate a new one if `CID_GENERATE = True`

### 6. `fluent-bit/fluent-bit.conf`

Main Fluent-bit pipeline configuration:
...
**Pipeline stages:**
1. **INPUT (forward):** Accept Docker logs
2. **FILTER (multiline):** Merge stack traces
3. **FILTER (parser):** Extract correlation_id and API fields
4. **FILTER (nest/lift):** Flatten docker labels into top-level fields
5. **OUTPUT (es):** Send parsed records to Elasticsearch

### 7. `fluent-bit/parsers.conf`

Parser definitions for regex and JSON extraction:
...
[PARSER]
    Name        uwsgi_perf_extract
    Format      regex
    Regex       ^(?<timestamp>\d{4}-\d{2}-\d{2}T[^\s]+)\s+(?<level>\w+)\s+\[uWSGI\]\s+(?<message>.*?)\s+(?<response_time>\d+\.?\d*ms)?
    Time_Key    timestamp
...
**Key parsers:**
- `docker_multiline`: Handles Python stack traces spanning multiple lines
- `uwsgi_perf_extract`: Extracts uWSGI access/perf logs (regex-based)
- `json_generic`: Parses JSON-formatted Django middleware logs
